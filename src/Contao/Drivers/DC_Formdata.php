...
class DC_Formdata extends \DataContainer implements \listable, \editable
{
    ...
    public function __construct($strTable, $arrModule = array())
    {
        ...
        if ($strTable == '' || !isset($GLOBALS['TL_DCA'][$strTable])) {
            $this->logger('Could not load the data container configuration for "' . $strTable . '"', __METHOD__,
                TL_ERROR);
    ...
    public function showAll()
    {
        ...
        if (strpos($GLOBALS['TL_DCA'][$this->strTable]['list']['sorting']['panelLayout'],
                'limit') !== false && ($strLimit = $this->limitMenu(true)) != false) {
            $return .= '
...
';
    ...
    public function show()
    {
        ...
        $sqlQuery = "SELECT * " . (!empty($this->arrSqlDetails) ? ', ' . implode(',',
                    array_values($this->arrSqlDetails)) : '') . " FROM " . $this->strTable . $table_alias;
        $sqlWhere = " WHERE id=?";
        if ($sqlWhere != '') {
            $sqlQuery .= $sqlWhere;
        }
        ...
        if (is_array($GLOBALS['TL_DCA'][$this->strTable]['fields'])) {
            $allowedFields = array_unique(array_merge($allowedFields,
                array_keys($GLOBALS['TL_DCA'][$this->strTable]['fields'])));
        }
...
<h2 class="sub_headline">' . sprintf($GLOBALS['TL_LANG']['MSC']['showRecord'],
                ($this->intId ? 'ID ' . $this->intId : '')) . '</h2>
...
<table class="tl_show">' . $return . '
    ...
    public function create($set = array())
    {
        ...
        if (!$GLOBALS['TL_DCA'][$this->strTable]['config']['closed']) {
            ...
            if ($objInsertStmt->affectedRows) {
                ...
                foreach ($this->arrDetailFields as $strDetailField) {
                    ...
                    if (isset($GLOBALS['TL_DCA'][$this->strTable]['fields'][$strDetailField]['default'])) {
                        ...
                        if (is_array($GLOBALS['TL_DCA'][$this->strTable]['fields'][$strDetailField]['default'])) {
                            $strVal = implode(',',
                                $GLOBALS['TL_DCA'][$this->strTable]['fields'][$strDetailField]['default']);
                ...
                // Add a log entry
                $this->logger('A new entry in table "' . $this->strTable . '" has been created (ID: ' . $insertID . ')',
    ...
    public function delete($blnDoNotRedirect = false)
    {
        ...
        $objUndoStmt = \Database::getInstance()->prepare("INSERT INTO tl_undo (pid, tstamp, fromTable, query, affectedRows, data) VALUES (?, ?, ?, ?, ?, ?)")
            ->execute($this->User->id, time(), $this->strTable,
                'DELETE FROM ' . $this->strTable . ' WHERE id=' . $this->intId, $affected, serialize($data));
        ...
        if ($objUndoStmt->affectedRows) {
            ...
            if ($this->strTable != 'tl_log') {
                $this->logger('DELETE FROM ' . $this->strTable . ' WHERE id=' . $data[$this->strTable][0]['id'],
                    __METHOD__, TL_GENERAL);
            }
        }
    ...
    public function undo()
    {
        ...
        if (!$error) {
            $this->logger('Undone ' . $query, __METHOD__, TL_GENERAL);
    ...
    public function edit($intID = null, $ajaxId = null)
    {
        ...
        if ($objRow->numRows < 1) {
            $this->logger('Could not load record "' . $this->strTable . '.id=' . $this->intId . '"', __METHOD__,
                TL_ERROR);
            \Controller::redirect('contao?act=error');
        }
            ...
            foreach ($boxes as $k => $v) {
                ...
                foreach ($v as $kk => $vv) {
                    ...
                    if ($GLOBALS['TL_DCA'][$this->strTable]['fields'][$this->strField]['eval']['multiple']
                        ...
                        $this->varValue = trimsplit($GLOBALS['TL_DCA'][$this->strTable]['fields'][$this->strField]['eval']['csv'],
                            $this->varValue);
                    }
                    ...
                    if (in_array($strInputType, array('radio', 'select', 'conditionalselect', 'countryselect'))
                        ...
                        if (in_array($this->strField, $this->arrBaseFields) && in_array($this->strField,
                                $this->arrOwnerFields)) {
                            ...
                            if ($strInputType == 'select' && strlen($GLOBALS['TL_DCA'][$this->strTable]['fields'][$this->strField]['foreignKey'])) {
                                ...
                                $arrKey          = explode('.',
                                    $GLOBALS['TL_DCA'][$this->strTable]['fields'][$this->strField]['foreignKey']);
                                ...
                                if (substr($strForeignTable, 0, 3) == 'fd_') {
                                    ...
                                    if (!empty($strForeignKeyCond)) {
                                        $arrForeignKeyCond   = preg_split('/([\s!=><]+)/', $strForeignKeyCond, -1,
                                            PREG_SPLIT_DELIM_CAPTURE);
                                        $strForeignCondField = $arrForeignKeyCond[0];
                                        unset($arrForeignKeyCond[0]);
                                        if (in_array($strForeignCondField,
                                            $GLOBALS['TL_DCA'][$strForeignTable]['tl_formdata']['baseFields'])) {
                                            $sqlForeignFd .= ' AND f.' . $strForeignCondField . implode('',
                                                    $arrForeignKeyCond);
                                        }
                                        if (in_array($strForeignCondField,
                                            $GLOBALS['TL_DCA'][$strForeignTable]['tl_formdata']['detailFields'])) {
                                            $sqlForeignFd .= ' AND (SELECT value FROM tl_formdata_details WHERE ff_name="' . $strForeignCondField . '" AND pid=f.id ) ' . implode('',
                                                    $arrForeignKeyCond);
                                        }
                                    }
                    ...
                    elseif ($strInputType == 'efgLookupRadio') {
                        ...
                        $this->varValue = $this->Formdata->prepareDatabaseValueForWidget($this->varValue,
                            $GLOBALS['TL_DCA'][$this->strTable]['fields'][$this->strField]);
                    }
...
<h2 class="sub_headline">' . sprintf($GLOBALS['TL_LANG']['MSC']['editRecord'],
                ($this->intId ? 'ID ' . $this->intId : '')) . '</h2>
' . \Message::generate() . '
<form action="' . ampersand(\Environment::get('request'),
                true) . '" id="' . $this->strTable . '" class="tl_form" method="post" enctype="' . ($this->blnUploadable ? 'multipart/form-data' : 'application/x-www-form-urlencoded') . '"' . (!empty($this->onsubmit) ? ' onsubmit="' . implode(' ',
                    $this->onsubmit) . '"' : '') . '>
...
<p class="tl_error">' . $GLOBALS['TL_LANG']['ERR']['general'] . '</p>' : '') . $return;
        ...
        if (\Input::post('FORM_SUBMIT') == $this->strTable && !$this->noReload) {
            ...
            \Database::getInstance()->prepare("UPDATE " . $this->strTable . " SET tstamp=? WHERE " . implode(' AND ',
                    $this->procedure))
                ->execute($arrValues);
...
<div class="' . $class . '">';
                ...
                $strSqlFields .= (!empty($arrSqlDetails) ? (strlen($strSqlFields) ? ', ' : '') . implode(', ',
                        $arrSqlDetails) : '');
                ...
                foreach ($this->strPalette as $v) {
                    ...
                    if ($v == '[EOF]') {
                        if ($blnAjax && \Environment::get('isAjaxRequest')) {
                            return $strAjax . '<input type="hidden" name="FORM_FIELDS_' . $id . '[]" value="' . specialchars(implode(',',
                                    $formFields)) . '">';
                        }
...
<form action="' . ampersand(\Environment::get('request'),
                    true) . '" id="' . $this->strTable . '" class="tl_form" method="post" enctype="' . ($this->blnUploadable ? 'multipart/form-data' : 'application/x-www-form-urlencoded') . '">
<div class="tl_formbody_edit">
...
<script>
  window.addEvent(\'domready\', function() {
    Backend.vScrollTo(($(\'' . $this->strTable . '\').getElement(\'label.error\').getPosition().y - 20));
            ...
            // Add meta fields if the current user is an administrator
            if ($this->User->isAdmin) {
                if (\Database::getInstance()->fieldExists('sorting', $this->strTable) && !in_array('sorting',
                        $fields)) {
...
<a href="' . $this->getReferer(true) . '" class="header_back" title="' . specialchars($GLOBALS['TL_LANG']['MSC']['backBTTitle']) . '" accesskey="b" onclick="Backend.getScrollOffset()">' . $GLOBALS['TL_LANG']['MSC']['backBT'] . '</a>
    ...
    protected function save($varValue)
    {
        ...
        if (!is_array($varValue) && strlen($varValue) && $arrField['eval']['unique']) {
            ...
            if ($objUnique->numRows) {
                throw new \Exception(sprintf($GLOBALS['TL_LANG']['ERR']['unique'],
                    (strlen($arrField['label'][0]) ? $arrField['label'][0] : $this->strField)));
            }
        }
        ...
        if (($varValue != '' || !$arrField['eval']['doNotSaveEmpty']) && ($this->varValue !== $varValue || $arrField['eval']['alwaysSave'])) {
            ...
            $sqlUpd = "UPDATE " . $strTargetTable . " SET " . $strTargetField . "=? WHERE " . implode(' AND ',
                    $arrProcedures);
            if ($blnDetailField) {
    ...
    public function getPalette()
    {
        ...
        if (!empty($GLOBALS['TL_DCA'][$this->strTable]['palettes']['__selector__'])) {
            ...
            $table_alias = ($this->strTable == 'tl_formdata' ? ' f' : '');
            $sqlQuery    = "SELECT * " . (!empty($this->arrSqlDetails) ? ', ' . implode(',',
                        array_values($this->arrSqlDetails)) : '') . " FROM " . $this->strTable . $table_alias . " WHERE id=?";
                ...
                foreach ($GLOBALS['TL_DCA'][$this->strTable]['palettes']['__selector__'] as $name) {
                    ...
                    if ($trigger != '') {
                        if (($GLOBALS['TL_DCA'][$this->strTable]['fields'][$name]['inputType'] == 'checkbox' || $GLOBALS['TL_DCA'][$this->strTable]['fields'][$name]['inputType'] == 'condition') && !$GLOBALS['TL_DCA'][$this->strTable]['fields'][$name]['eval']['multiple']) {
                            ...
                            $key = $name . '_' . array_search($trigger,
                                    $GLOBALS['TL_DCA'][$this->strTable]['fields'][$name]['options']);
            ...
            foreach ($subpalettes as $k => $v) {
                $strPalette = preg_replace('/\b' . preg_quote($k, '/') . '\b/i', $k . ',[' . $k . '],' . $v . ',[EOF]',
                    $strPalette);
            }
    ...
    protected function reviseTable()
    {
        ...
        if (isset($GLOBALS['TL_HOOKS']['reviseTable']) && is_array($GLOBALS['TL_HOOKS']['reviseTable'])) {
            foreach ($GLOBALS['TL_HOOKS']['reviseTable'] as $callback) {
                ...
                if (is_array($callback)) {
                    $this->import($callback[0]);
                    $status = $this->{$callback[0]}->{$callback[1]}($this->strTable, $new_records[$this->strTable],
                        $ptable, $ctable);
                } elseif (is_callable($callback)) {
        ...
        if (!empty($new_records[$this->strTable]) && is_array($new_records[$this->strTable])) {
            $objStmt = \Database::getInstance()->execute("DELETE FROM " . $this->strTable . " WHERE id IN(" . implode(',',
                    array_map('intval', $new_records[$this->strTable])) . ") AND tstamp=0");
    ...
    protected function listView()
    {
        ...
        if (\Input::get('table') && $GLOBALS['TL_DCA'][$this->strTable]['config']['ptable'] && \Database::getInstance()->fieldExists('pid',
                $this->strTable)) {
            $this->procedure[] = 'pid=?';
            $this->values[]    = \Input::get('id');
        }

        $query = "SELECT * " . (!empty($this->arrSqlDetails) ? ', ' . implode(',',
                    array_values($this->arrSqlDetails)) : '') . " FROM " . $this->strTable . $table_alias;
...
<a href="' . $this->getReferer(true,
                        $this->ptable) . '" class="header_back" title="' . specialchars($GLOBALS['TL_LANG']['MSC']['backBTTitle']) . '" accesskey="b" onclick="Backend.getScrollOffset()">' . $GLOBALS['TL_LANG']['MSC']['backBT'] . '</a> ' : (isset($GLOBALS['TL_DCA'][$this->strTable]['config']['backlink']) ? '
...
<table class="tl_listing' . ($GLOBALS['TL_DCA'][$this->strTable]['list']['label']['showColumns'] ? ' showColumns' : '') . '">';
            ...
            if ($GLOBALS['TL_DCA'][$this->strTable]['list']['label']['showColumns'] && !in_array($firstOrderBy,
                    $GLOBALS['TL_DCA'][$this->strTable]['list']['label']['fields'])) {
            ...
            foreach ($result as $row) {
                ...
                foreach ($showFields as $k => $v) {
                    if (in_array($v, $this->arrDetailFields)
                        && in_array($GLOBALS['TL_DCA'][$this->strTable]['fields'][$v]['inputType'], array(
                            'radio',
                            'efgLookupRadio',
                            'select',
                            'efgLookupSelect',
                    ...
                    if (in_array($v,
                            $this->arrDetailFields) && $GLOBALS['TL_DCA'][$this->strTable]['fields'][$v]['eval']['multiple']) {
                        $strSep  = isset($GLOBALS['TL_DCA'][$this->strTable]['fields'][$v]['eval']['csv']) ? $GLOBALS['TL_DCA'][$this->strTable]['fields'][$v]['eval']['csv'] : '|';
                        $row[$v] = str_replace($strSep, ', ', $row[$v]);
                    }
                    ...
                    if (in_array($GLOBALS['TL_DCA'][$this->strTable]['fields'][$v]['flag'], array(5, 6, 7, 8, 9, 10))) {
                        if ($GLOBALS['TL_DCA'][$this->strTable]['fields'][$v]['eval']['rgxp'] == 'date') {
                            $args[$k]         = strlen($row[$v]) ? \Date::parse($GLOBALS['TL_CONFIG']['dateFormat'],
                                $row[$v]) : '-';
                            $rowFormatted[$v] = $args[$k];
                        } elseif ($GLOBALS['TL_DCA'][$this->strTable]['fields'][$v]['eval']['rgxp'] == 'time') {
                            $args[$k]         = strlen($row[$v]) ? \Date::parse($GLOBALS['TL_CONFIG']['timeFormat'],
                                $row[$v]) : '-';
                            $rowFormatted[$v] = $args[$k];
                        } else {
                            $args[$k]         = strlen($row[$v]) ? \Date::parse($GLOBALS['TL_CONFIG']['datimFormat'],
                                $row[$v]) : '-';
                            $rowFormatted[$v] = $args[$k];
                        }
                ...
                $label = vsprintf((strlen($GLOBALS['TL_DCA'][$this->strTable]['list']['label']['format']) ? $GLOBALS['TL_DCA'][$this->strTable]['list']['label']['format'] : '%s'),
                    $args);
                ...
                if ($GLOBALS['TL_DCA'][$this->strTable]['list']['label']['maxCharacters'] > 0 && $GLOBALS['TL_DCA'][$this->strTable]['list']['label']['maxCharacters'] < strlen(strip_tags($label))) {
                    ...
                    if (version_compare(VERSION . '.' . BUILD, '3.5.5', '>=')) {
                        $label = trim(\StringUtil::substrHtml($label,
                                $GLOBALS['TL_DCA'][$this->strTable]['list']['label']['maxCharacters'])) . ' …';
                    } else {
                        $label = trim(\String::substrHtml($label,
                                $GLOBALS['TL_DCA'][$this->strTable]['list']['label']['maxCharacters'])) . ' …';
                    }
                }
    ...
    ';
                ...
                if (is_array($GLOBALS['TL_DCA'][$this->strTable]['list']['label']['label_callback']) || is_callable($GLOBALS['TL_DCA'][$this->strTable]['list']['label']['label_callback'])) {
                    if (is_array($GLOBALS['TL_DCA'][$this->strTable]['list']['label']['label_callback'])) {
                        ...
                        $args = $GLOBALS['TL_DCA'][$this->strTable]['list']['label']['label_callback']($row, $label,
                            $this, $args);
                    }
    ...
    protected function searchMenu()
    {
        ...
        if (\Input::post('FORM_SUBMIT') == 'tl_filters') {
            ...
            if (\Input::postRaw('tl_value') != '') {
                $sqlSearchField = '(SELECT value FROM tl_formdata_details WHERE ff_name=\'' . \Input::post('tl_field',
                        true) . '\' AND pid=f.id)';
                try {
                    \Database::getInstance()->prepare("SELECT * " . (!empty($this->arrSqlDetails) ? ',' . implode(', ',
                                array_values($this->arrSqlDetails)) : '') . " FROM " . $this->strTable . " f WHERE " . $sqlSearchField . " REGEXP ?")
                        ->limit(1)
...
<input type="text" name="tl_value" class="tl_text' . ($active ? ' active' : '') . '" value="' . specialchars($session['search'][$strSessionKey]['value']) . '">
    ...
    protected function sortMenu()
    {
        ...
        if ($GLOBALS['TL_DCA'][$this->strTable]['list']['sorting']['mode'] == 3 && \Database::getInstance()->fieldExists('pid',
                $this->strTable)) {
        ...
        if (\Input::post('FORM_SUBMIT') == 'tl_filters') {
            $session['sorting'][$strSessionKey] = in_array($GLOBALS['TL_DCA'][$this->strTable]['fields'][\Input::post('tl_sort')]['flag'],
                array(2, 4, 6, 8, 10, 12)) ? \Input::post('tl_sort') . ' DESC' : \Input::post('tl_sort');
            try {
                $this->Session->setData($session);
            } catch (\Exception $ex) {
                $this->logger('For Session setData occured an error', __METHOD__, TL_ERROR);
        ...
        foreach ($sortingFields as $field) {
            ...
            $options_sorter[$options_label] = '  <option value="' . specialchars($field) . '"' . ((!strlen($session['sorting'][$strSessionKey]) && $field == $firstOrderBy || $field == str_replace(' DESC',
                        '',
                        $session['sorting'][$strSessionKey])) ? ' selected="selected"' : '') . '>' . $options_label . '</option>';
        }
...
' . implode("\n", $options_sorter) . '
    ...
    protected function limitMenu($blnOptional = false)
    {
        ...
        if (\Input::post('FORM_SUBMIT') == 'tl_filters' || \Input::post('FORM_SUBMIT') == 'tl_filters_limit') {
            ...
            // Overall limit
            if ($total > $GLOBALS['TL_CONFIG']['maxResultsPerPage'] && ($this->limit === null || preg_replace('/^.*,/',
                        '', $this->limit) == $GLOBALS['TL_CONFIG']['maxResultsPerPage'])) {
                if ($this->limit === null) {
            ...
            if ($total > 0) {
                ...
                if ($this->limit !== null && ($this->limit == '' || preg_replace('/,.*$/', '',
                            $this->limit) > $total)) {
                ...
                for ($i = 0; $i < $options_total; $i++) {
                    ...
                    $options .= '
  <option value="' . $this_limit . '"' . \Widget::optionSelected($this->limit,
                            $this_limit) . '>' . ($i * $GLOBALS['TL_CONFIG']['resultsPerPage'] + 1) . ' - ' . $upper_limit . '</option>';
                }

                if (!$blnIsMaxResultsPerPage) {
                    $options .= '
  <option value="all"' . \Widget::optionSelected($this->limit,
                            null) . '>' . $GLOBALS['TL_LANG']['MSC']['filterAll'] . '</option>';
                }
            }
...
<strong>' . $GLOBALS['TL_LANG']['MSC']['showOnly'] . ':</strong> ' . $fields . '
    ...
    protected function filterMenu($intFilterPanel)
    {
                        ...
                        if (isset($GLOBALS['TL_DCA'][$this->strTable]['fields'][$field]['eval']['csv'])) {
                            if (in_array($field, $this->arrDetailFields)) {
                                $this->procedure[] = \Database::getInstance()
                                    ->findInSet('?',
                                        "(SELECT REPLACE(`value`, '" . $GLOBALS['TL_DCA'][$this->strTable]['fields'][$field]['eval']['csv'] . "', ',') FROM tl_formdata_details WHERE ff_name='" . $field . "' AND pid=f.id)",
                                        true);
        ...
        foreach ($sortingFields as $cnt => $field) {
            ...
            $objFields = \Database::getInstance()->prepare("SELECT DISTINCT(" . $sqlField . ") AS `" . $field . "` FROM " . $this->strTable . " f " . ((is_array($arrProcedure) && strlen($arrProcedure[0])) ? ' WHERE ' . implode(' AND ',
                        $arrProcedure) : ''))
                ->execute($arrValues);
            ...
            if ($objFields->numRows) {
                ...
                if ($GLOBALS['TL_DCA'][$this->strTable]['fields'][$field]['eval']['multiple']) {
                    ...
                    foreach ($options as $option) {
                        // CSV lists (see #2890)
                        if (isset($GLOBALS['TL_DCA'][$this->strTable]['fields'][$field]['eval']['csv'])) {
                            $doptions = trimsplit($GLOBALS['TL_DCA'][$this->strTable]['fields'][$field]['eval']['csv'],
                                $option);
                        } else {
                ...
                $options_sorter = array();
                $blnDate        = in_array($GLOBALS['TL_DCA'][$this->strTable]['fields'][$field]['flag'],
                    array(5, 6, 7, 8, 9, 10));
...
<strong>' . $GLOBALS['TL_LANG']['MSC']['filter'] . ':</strong> ' . $fields . '
    ...
    protected function formatCurrentValue($field, $value, $mode)
    {
        ...
        if ($GLOBALS['TL_DCA'][$this->strTable]['fields'][$field]['inputType'] == 'checkbox' && !$GLOBALS['TL_DCA'][$this->strTable]['fields'][$field]['eval']['multiple']) {
            ...
            $remoteNew = ($value != '') ? ucfirst(utf8_substr($value, 0,
                $GLOBALS['TL_DCA'][$this->strTable]['fields'][$field]['length'])) : '-';
    ...
    protected function formatGroupHeader($field, $value, $mode, $row)
    {
        ...
        if (is_array($GLOBALS['TL_DCA'][$this->strTable]['list']['label']['group_callback'])) {
            ...
            $group = $GLOBALS['TL_DCA'][$this->strTable]['list']['label']['group_callback']($group, $mode, $field, $row,
                $this);
        }
    ...
    public function mail($intID = false)
    {
        ...
        if ($objForm == null) {
            $this->logger('Could not load record "tl_form.id=' . $intFormId . '" / "tl_form.title=' . $arrSubmitted['form'] . '"',
                __METHOD__, TL_ERROR);
            \Controller::redirect('contao?act=error');
        }
        ...
        if (!empty($recipientFieldName) && !empty($arrSubmitted[$recipientFieldName])) {
            ...
            if (in_array($arrFormFields[$recipientFieldName]['type'], array('radio', 'select', 'checkbox'))) {
                if (!$blnStoreOptionsValues) {
                    $arrRecipient = $this->Formdata->prepareDatabaseValueForWidget($varRecipient,
                        $arrFormFields[$recipientFieldName], false);
        ...
        if (strlen($varRecipient) || strlen($arrForm['confirmationMailRecipient'])) {
            $arrRecipient = array_merge(trimsplit(',', $varRecipient),
                trimsplit(',', $arrForm['confirmationMailRecipient']));
        }
        ...
        if (is_array($arrRecipient)) {
            ...
            if (!is_bool(strpos($strRecipient, "{{user::email}}")) && $arrSubmitted['fd_member'] > 0) {
                ...
                $arrRecipient = array_map("str_replace", array_fill(0, count($arrRecipient), "{{user::email}}"),
                    array_fill(0, count($arrRecipient), $objUser->email), $arrRecipient);
            }
        ...
        $objMailProperties = $this->Formdata->prepareMailData($objMailProperties, $arrSubmitted, $arrFiles, $arrForm,
            $arrFormFields);
            ...
            if ($blnSend) {
                ...
                if (!empty($objMailProperties->recipients)) {
                    ...
                    foreach ($objMailProperties->recipients as $recipient) {
                        ...
                        \Message::addInfo(sprintf($GLOBALS['TL_LANG']['tl_formdata']['mail_sent'],
                            str_replace(array('<', '>'), array('[', ']'), $recipient)));
                    }
                }
        ...
        if (strlen($objRow->confirmationSent)) {
            if (!$blnSend) {
                if (strlen($objRow->confirmationDate)) {
                    ...
                    $strHint          .= '<div class="tl_message"><p class="tl_info">' . sprintf($GLOBALS['TL_LANG']['tl_formdata']['confirmation_sent'],
                            $dateConfirmation->date, $dateConfirmation->time) . '</p></div>';
                } else {
                    $strHint .= '<div class="tl_message"><p class="tl_info">' . sprintf($GLOBALS['TL_LANG']['tl_formdata']['confirmation_sent'],
                            '-n/a-', '-n/a-') . '</p></div>';
                }
...
<table cellpadding="0" cellspacing="0" class="prev_header" summary="">
  ...
  <tr class="row_1">
    ...
    <td class="col_1"><input name="recipient" type="ctrl_recipient" class="tl_text" value="' . implode(',',
                $objMailProperties->recipients) . '" ' . ($blnSend ? 'disabled="disabled"' : '') . '></td>
  </tr>
...
<input type="submit" id="send" class="tl_submit" accesskey="s" value="' . specialchars($GLOBALS['TL_LANG']['tl_formdata']['mail'][0]) . '">
    ...
    public function importFile()
    {
        ...
        if (null === $this->arrImportIgnoreFields) {
            $this->arrImportIgnoreFields = array(
                'id',
                'pid',
                'tstamp',
                'form',
        ...
        if ($_POST['FORM_SUBMIT'] == 'tl_formdata_import') {
            ...
            if ($_POST['FORM_MODE'] == 'import') {
                ...
                $strAliasField = (strlen($this->Formdata->arrStoringForms[substr($this->strFormKey,
                    3)]['efgAliasField']) ? $this->Formdata->arrStoringForms[substr($this->strFormKey,
                    3)]['efgAliasField'] : '');
                    ...
                    foreach ($arrMapFields as $strField => $intCol) {
                        ...
                        if (in_array($strField, $this->arrBaseFields)) {
                            ...
                            if (in_array($strField, $this->arrOwnerFields)) {
                                ...
                                if (is_numeric($arrRow[$intCol]) && array_key_exists($arrRow[$intCol],
                                        $this->{$array})) {
                ...
                // Add a log entry
                $this->logger('Imported file "' . $objFile->filename . '" into form data "' . $strFormTitle . '", created ' . $intValid . ' new records',
                    __METHOD__, TL_GENERAL);
    ...
    protected function formImportSource()
    {
        ...
        $objFileSelector = new \FileTree(\Widget::getAttributesFromDca($GLOBALS['TL_DCA']['tl_formdata']['fields']['import_source'],
            'import_source', null, 'import_source', 'tl_formdata'));

        // Return form
        return '
<div id="tl_buttons">
<a href="' . ampersand(str_replace('&key=import', '',
                \Environment::get('request'))) . '" class="header_back" title="' . specialchars($GLOBALS['TL_LANG']['MSC']['backBT']) . '" accesskey="b">' . $GLOBALS['TL_LANG']['MSC']['backBT'] . '</a>
</div>
...
<select name="import_cols[' . $col . ']">
    ...
    public function export($strMode = 'csv')
    {
        ...
        if ($strMode == 'exportxls')  //TODO XLS EXPORT intigrieren???
        {
            ...
            if (array_key_exists('efgExportXls',
                    $GLOBALS['TL_HOOKS']) && is_array($GLOBALS['TL_HOOKS']['efgExportXls'])) {
        ...
        if ($strMode == 'csv') {
            ...
            if (!$blnCustomXlsExport) {
                ...
                $exportsheet = new Worksheet($spreadsheet, 'Export');
                try {
                    $spreadsheet->addSheet($exportsheet, 0);
                } catch (\Exception $ex) {
                    $exportsheet->setTitle('Export' . date("Ymd_His"));
                    try {
                        $spreadsheet->addSheet($exportsheet, 0);
                    } catch (\Exception $finalex) {
                        //TODO Handele finale Exception
                    }
                }
        ...
        if ($objRow->numRows) {
            ...
            foreach ($result as $row) {
                ...
                if ($intRowCounter == 1) {
                    if ($strMode == 'xls') {
                        if (!$blnCustomXlsExport) {
                            //TODO Is needed in phpspreadsheet? $spreadsheet->???
...
//							$strName = $this->convertEncoding($strName, $GLOBALS['TL_CONFIG']['characterSet'], $this->strExportConvertToCharset);
                        ...
                        if ($strMode == 'csv') {
                            ...
                            if (!$blnCustomXlsExport) {
                                try {
                                    $spreadsheet->getActiveSheet()->setCellValueByColumnAndRow(($intColCounter + 1),
                                        $intRowCounter, $strName);
                                    $spreadsheet->getActiveSheet()->getStyleByColumnAndRow(($intColCounter + 1),
                                        $intRowCounter)->getFill()->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID);
                                } catch (Exception $e) {
                                    //TODO Handle Exception
                                }
                ...
                foreach ($showFields as $k => $v) {
                    ...
                    if ($GLOBALS['TL_DCA'][$this->strTable]['fields'][$v]['inputType'] == 'date'
                        && in_array($GLOBALS['TL_DCA'][$this->strTable]['fields'][$v]['flag'],
                            array(5, 6, 7, 8, 9, 10))) {
                    ...
                    } elseif (in_array($GLOBALS['TL_DCA'][$this->strTable]['fields'][$v]['flag'],
                        array(5, 6, 7, 8, 9, 10))) {
                        $strVal = ($row[$v] ? date($GLOBALS['TL_CONFIG']['datimFormat'], $row[$v]) : '');
                    } elseif ($GLOBALS['TL_DCA'][$this->strTable]['fields'][$v]['inputType'] == 'checkbox'
                              && !$GLOBALS['TL_DCA'][$this->strTable]['fields'][$v]['eval']['multiple']) {
                        ...
                        if (!empty($GLOBALS['TL_DCA'][$this->strTable]['fields'][$v]['eval']['csv'])) {
                            $row_v = explode($GLOBALS['TL_DCA'][$this->strTable]['fields'][$v]['eval']['csv'],
                                $row[$v]);
                        }
     ...
     * @return String
     *
     * @param $strString String charset to convert
     * @param $from      string charsetto convert from
     * @param $to        string charset to convert to
     */
    public function convertEncoding($strString, $from, $to)
    {
        ...
        if (extension_loaded('mbstring')) // ORIGINAL USE_MBSTRING)
        {
