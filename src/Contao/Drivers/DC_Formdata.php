...
class DC_Formdata extends \DataContainer implements \listable, \editable
{
    ...
    public function __construct($strTable, $arrModule=array())
    {
        ...
        {
            $this->logger('Could not load the data container configuration for "' . $strTable . '"', __METHOD__, TL_ERROR);
...
<table class="tl_show">'.$return.'
    ...
    public function create($set=array())
    {
        ...
        if (!$GLOBALS['TL_DCA'][$this->strTable]['config']['closed'])
        {
            ...
            if ($objInsertStmt->affectedRows)
            {
                ...
                // Add a log entry
                $this->logger('A new entry in table "'.$this->strTable.'" has been created (ID: '.$insertID.')', __METHOD__, TL_GENERAL);
    ...
    public function delete($blnDoNotRedirect=false)
    {
        ...
        if ($objUndoStmt->affectedRows)
        {
            ...
            {
                $this->logger('DELETE FROM '.$this->strTable.' WHERE id='.$data[$this->strTable][0]['id'], __METHOD__, TL_GENERAL);
            }
        }
    ...
    public function importFile()
    {
        ...
        if ($_POST['FORM_SUBMIT'] == 'tl_formdata_import')
        {
            ...
            if ($_POST['FORM_MODE'] == 'import')
            {
                ...
                // Add a log entry
                $this->logger('Imported file "'.$objFile->filename.'" into form data "'.$strFormTitle.'", created '.$intValid.' new records', __METHOD__, TL_GENERAL);
            ...
            foreach ($result as $row)
            {
                ...
                if ($intRowCounter == 1)
                {
                    ...
                    foreach ($showFields as $k=>$v)
                    {
                        ...
                        elseif ($strMode == 'exportxls' || $strMode == 'xls')
                        {
                            ...
                            {
                                try {
                                    $spreadsheet->getActiveSheet()->setCellValueByColumnAndRow(($intColCounter + 1), $intRowCounter, $strName);
                                    $spreadsheet->getActiveSheet()->getStyleByColumnAndRow(($intColCounter + 1), $intRowCounter)->getFill()->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID);
                                }catch(Exception $e) {
                                    //TODO Handle Exception
                                }
                ...
                foreach ($showFields as $k=>$v)
                {
                    ...
                    elseif ($strMode == 'exportxls' || $strMode == 'xls')
                    {
                        ...
                        {
                            try {
                                $spreadsheet->getActiveSheet()->setCellValueByColumnAndRow(($intColCounter + 1), $intRowCounter, $strVal);
                            }catch (Exception $e) {
                                //TODO Handle Exception
                            }
