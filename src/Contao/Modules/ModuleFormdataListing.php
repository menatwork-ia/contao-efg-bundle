...
use MenAtWork\EfgBundle\Contao\Forms\ExtendedForm;
...
class ModuleFormdataListing extends \Module
{
	...
	protected function compile()
	{
		...
		$this->strIconFolder = (strlen($this->efg_iconfolder) ? $this->efg_iconfolder : 'bundles/efg');
		...
		if ($blnExport)
		{
			...
			if ($strExportMode=='xls')
			{
				...
				{
					include(TL_ROOT.'/plugins/xls_export/xls_export.php'); // TODO CHECK XLS_EXPORTER
				}
			}
		}
		else
		...
		else
		{
			...
			if ($strExportMode=='xls')
			{
				if (!$blnCustomXlsExport && false)
				{
					$xls = new xlsexport();     //TODO Check if still needed?
                    $strXlsSheet = "Export";
	...
	protected function exportSingleRecord($strExportMode='csv')
	{
		...
		if ($strExportMode == 'xls')
		{
			...
			if (!$blnCustomXlsExport) //TODO XLS EXport
			{
				$xls = new xlsexport();
	...
	public function generateEditForm($objFormElement, $objRecord)
	{
		...
		$this->EditForm->_set('objEditRecord' ,$objRecord);
	 ...
	 * @param $from string  charset to convert from
