...
namespace MenAtWork\EfgBundle\Contao;
...
class FormdataProcessor extends \Frontend
{
    ...
    public function processSubmittedData($arrSubmitted, $arrForm = false, $arrFiles = false, $arrLabels = false)
    {
        ...
        $intListingId = intval(($_SESSION['EFP']['LISTING_MOD']['id'] ?? 0));
        if ($intListingId > 0) {
        ...
        if (!empty($arrListing['efg_fe_edit_access']) && in_array($arrListing['efg_fe_edit_access'], array('public', 'groupmembers', 'member'))) {
        ...
        if ($arrForm['storeFormdata'] && !empty($arrSubmitted)) {
            ...
            (
                ...
                'published' => (!empty($GLOBALS['TL_DCA']['tl_formdata']['fields']['published']['default']) ? '1' : ''),
                'fd_member' => intval($this->Member->id),
                'fd_member_group' => intval(($this->Member->groups[0] ?? 0)),
                'fd_user' => intval(($this->User->id ?? 0)),
                'fd_user_group' => intval(($this->User->groups[0] ?? 0))
            );
            ...
            if ($blnFEedit && strlen($arrListing['efg_fe_keep_id'])) {
                ...
                // TODO CHANGE because deprecated - $bundles = array_keys(\System::getContainer()->getParameter('kernel.bundles'));
            ...
            foreach ($arrFormFields as $k => $arrField) {
                ...
                if (in_array($strType, $arrFFstorable)) {
                    ...
                    $strVal = $this->Formdata->preparePostValueForDatabase(($arrSubmitted[$k] ?? null), $arrField, ($arrFiles[$k] ?? null));
        ...
        foreach ($arrFormFields as $k => $arrField) {
            ...
            $strVal = '';
            if (in_array($strType, $arrFFstorable)) {
                $strVal = $this->Formdata->preparePostValueForMail($arrSubmitted[$k], $arrField, ($arrFiles[$k]??null), $blnSkipEmptyFields);
            }
        ...
        if ($arrForm['sendFormattedMail']) {
            ...
            if (!empty($arrSubmitted[$senderFieldName]) && !is_array($arrSubmitted[$senderFieldName])) {
                ...
                // PHP 7 compatibility, see https://github.com/contao/core-bundle/issues/309
                if (version_compare(VERSION . '.' . BUILD, '3.5.5', '>=')) {
                    list($senderName, $sender) = \StringUtil::splitFriendlyEmail($this->replaceInsertTags($arrForm['formattedMailSender'], false));
                } else {
